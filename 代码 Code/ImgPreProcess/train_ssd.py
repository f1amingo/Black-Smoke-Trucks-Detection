...
if platform.system() == 'Windows':
    DATA_SET_ROOT = 'C:/Users/think/Desktop/VOCtemplate'
    MODEL_ROOT = '~/MyFiles/.mxnet/models'
else:
    DATA_SET_ROOT = '/home/ubuntu/MyFiles/VOCtemplate'
    MODEL_ROOT = '~/MyFiles/.mxnet/models'
...
def get_dataset(dataset, args):
    ...
    elif dataset.lower() == 'smoke':
        train_dataset = VOCLike(root=DATA_SET_ROOT, splits=((2019, 'train1'),))
        val_dataset = VOCLike(root=DATA_SET_ROOT, splits=((2019, 'test1'),))
        val_metric = VOC07MApMetric(iou_thresh=0.5, class_names=val_dataset.classes)
...
if __name__ == '__main__':
    ...
    if args.syncbn and len(ctx) > 1:
        net = get_model(net_name, pretrained=True, norm_layer=gluon.contrib.nn.SyncBatchNorm,
                        root='~/MyFiles/.mxnet/models',
                        norm_kwargs={'num_devices': len(ctx)})
        async_net = get_model(net_name, pretrained_base=False, root='~/MyFiles/.mxnet/models')  # used by cpu worker
    else:
        net = get_model(net_name, pretrained=True, norm_layer=gluon.nn.BatchNorm, root='~/MyFiles/.mxnet/models')
        async_net = net
    ...
    net.reset_class(['smoke'])

    # training data
    train_dataset, val_dataset, eval_metric = get_dataset('smoke', args)

    print('train len: %d, test len: %d' % (len(train_dataset), len(val_dataset)))
